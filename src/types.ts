...
export interface SpecResult {
  ...
  deprecationWarnings: jasmine.FailedExpectation[];
...
export declare namespace jasmine {
  ...
  function objectContaining<T>(
    sample: { [K in keyof T]?: ExpectedRecursive<T[K]> }
  ): ObjectContaining<T>;
  ...
  interface RunDetails {
    overallStatus: any; // added by wolv
  ...
  interface Spec extends SuiteOrSpec {
    ...
    result: SpecResult; // added by wolv
    status: () => 'passed' | 'failed' | 'pending'; // added by wolv
  ...
  interface Suite extends SuiteOrSpec {
    ...
    specs(): Spec[];
    suites(): Suite[];
    children: { [id: string]: Spec | Suite }; // added by wolv
    // children(): any[]; // original
    execute(onComplete?: () => void): void;
  }
